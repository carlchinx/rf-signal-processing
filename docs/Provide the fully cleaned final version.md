...
# From Bog-Standard to Deep Characterisation
...
The first test I set myself was straightforward. Could I still load the files, extract the standard metrics, and produce a coherent characterisation? This is the bedrock of RF engineering competency — the fluency that separates practitioners from students — and I wanted to know it was still there. This is not a trivial question. This is a necessary skill to ground ourselves and to verify that we are not just throwing more tools at the problem, but that we can still do the basics without them and check if the results make sense.

The bottomline: the skill is still there. The standard analysis came back without difficulty.

> **Stage 1 — Standard Characterisation** *· [interactive viewer](sparam_viewer-sanitised.html)*
>
> S21 and S11 overlaid across all four units, sweeping 57 MHz to 10 GHz; passband zoom to 3–5 GHz; per-unit comparison cards showing peak insertion loss, centre frequency, −3 dB bandwidth, and stopband rejection at 1 GHz; traces togglable by unit, values readable on hover (*see sparam_viewer-sanitised.html in GitHub*).
>
> This is the conventional deliverable. Competent, complete, and correct. It answered every question a client would have asked in 2017.
...
| :-- | :-- |
| Peak S21 | −0.60 to −0.83 dB  |
| −3 dB bandwidth | 655–743 MHz  |
| Stopband rejection at 1 GHz | −106 to −115 dB  |
| Passband return loss | Better than −10 dB (typical)  |
...
## Part Two: Research-Level Characterisation
...
What this communicates to a non-specialist is immediate. This device is not instantaneous. Feed it something short, and it will ring for over ten times the pulse duration.
...
In the passband, the partition runs approximately 57% transmitted, 30% reflected, and 13% dissipated as heat.  In the stopband, the picture flips: over 95% of incident power is reflected, with little absorbed.  This confirms the device as a reactive, not absorptive, filter topology.
...
This is immediately useful intelligence for a production engineer. Not “units vary by 500 MHz,” which the standard overlay already shows, but “units vary at the band edges because of edge-coupling tolerances, and the flat core is robust to whatever manufacturing spread you have.”
...
## Part Three: Deep Characterisation

> *“More visualisations and depth. Deeper analysis.”*

The second push produced nine panels of analysis that move from characterisation into investigation — from measuring what the filter does to understanding why, and from describing the batch to decomposing it.
...
For the units under test, this analysis reveals a consistent two-pole structure, with poles appearing at approximately 3.7 GHz and 4.3 GHz and with unit-to-unit variation of about ±200–300 MHz.  Loaded Q values run from roughly 40 to 90.

The key observation is the vertical spread in the pole map: the 500 MHz unit-to-unit centre-frequency variation is not a statistical artefact. It is the combined detuning of two resonator pairs shifting together.
...
Fitting a straight line to the unwrapped S21 phase within the passband and plotting the residuals extracts the phase distortion directly.  For all four units, the peak-to-peak phase ripple within the passband is 78–92°, with RMS values of 11–13°.

To translate this: a perfect filter would preserve the shape of any signal within its band. This filter does not — and the residuals say precisely how much it does not, and where the deviation is worst.

### Gaussian Pulse Propagation — Simulation from Measurement
...
>
> Unit 4: **3.30 ns** delay and **−6.53 dB** insertion loss, about 3 dB worse than the best unit.

That −6.53 dB figure for Unit 4 is not visible in the standard S21 plot. The CW insertion loss at 4.2 GHz looks fine at about −0.63 dB, but the filter's offset centre frequency clips the edges of a 600 MHz pulse asymmetrically.  For a pulsed system with a tight link budget, that distinction matters.

### Principal Component Analysis — The Mode of Failure

Applying singular value decomposition to the 4 × 1,530 matrix of S21 responses decomposes the inter-unit variation into orthogonal modes.  The result resolves a question the overlay plot raises but does not answer: is the batch variation primarily a frequency shift, a bandwidth change, an insertion-loss drift, or something more complex?

> **PCA result**
>
> **92%** of all unit-to-unit variance is captured in a single mode, PC1.
>
...
The engineering implication is direct. If you are tightening the production specification, most of the problem sits in a single process parameter — whatever dimension or material property sets the resonant frequency of the cavities.
...
| Dominant Q$_L$ range | 59–91  | 74–80  | 74–84  | 81–83  |
...
The comparison is not simply “fewer outputs then, more outputs now.” It is a change in the economics of depth.
...
| Batch variation | Visual range on overlay  | PCA: 92% of variance = frequency-shift mode  |
...
**Production variation is a physics question, not just a statistics question.** The PCA result — that 92% of the inter-unit variance in this batch is a single frequency-shift mode — is a statement about the physical mechanism of manufacturing variability.
...
Stage 1 confirmed the tick — the standard competency is intact. Stage 2 added the temporal, energetic, and impedance story that the amplitude plots do not tell. Stage 3 went further: extracted the physical architecture of the device from its measured response, simulated real-signal performance, and decomposed eight years of untouched batch variation into a single actionable insight about the dominant manufacturing mode.

The device under test is, in the end, a well-designed filter. Good insertion loss, excellent lower stopband rejection, and consistent impedance matching.  The 500 MHz centre-frequency spread across the batch is the headline finding from 2017, and it remains the headline finding now.

What changed is everything around it: the group-delay peak at the band edges, the 12-nanosecond ringing tail, the −6.5 dB pulse hit on Unit 4, and the 92% of variance sitting in one frequency-shift mode.  These are the things the data was quietly carrying for eight years, waiting to be asked.

The bog-standard analysis was the right starting point. It was just never supposed to be the ending one.
